...
def connect() -> bool:
    ...
    try:
        ...
        connection = pyodbc.connect(
            ...
            database = "BD23304", #banco de dados
            uid = "BD23304", #LOGIN
            pwd = "BD23304" #SENHA
        ) # xxxxx é seu RA
        return True
...
def procurar (nom,sistema = False):
    ...
    while not digitouDireito:
        ...
        resposta=incluir(nom,nome)
        achou   = resposta[0]
        posicao = resposta[1]
        
        if not achou:
            print ('Pessoa inexistente - Por favor faça cadastro...')
            continue
            # incluir(agd)
        else:
            digitouDireito=True
            dados_selecionados = posicao
...
while opcao!=6:
    ...
    if opcao==1:
        incluir(sucessoNoAcessoAoBD)
    elif opcao==2:
        procurar(sucessoNoAcessoAoBD)
    elif opcao==3:
        atualizar(sucessoNoAcessoAoBD)
    elif opcao==4:
        listar(sucessoNoAcessoAoBD)
    elif opcao==5:
        excluir(sucessoNoAcessoAoBD)
